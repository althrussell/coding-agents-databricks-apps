...
if token:
    ...
    # Plugin marketplace, theme, permissions, and hooks (from coda-essentials).
    # Assigned per-key on top of the read-merge-write env block above so we
    # preserve the PR #153 race fix — overwriting `settings` wholesale would
    # drop env vars that other setup scripts (e.g. setup_mlflow.py) wrote.
    settings["theme"] = "dark"
    settings["outputStyle"] = "Explanatory"
    settings.setdefault("extraKnownMarketplaces", {})
    settings["extraKnownMarketplaces"]["coda"] = {
        "source": {
            "source": "directory",
    ...
    settings["enabledPlugins"]["coda-essentials@coda"] = True
    settings["enabledPlugins"]["coda-databricks-skills@coda"] = True
