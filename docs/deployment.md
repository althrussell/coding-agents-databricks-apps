...
| `CODA_AUTO_MODE` | No | Claude "auto mode" (`permissions.defaultMode`). **Lab profile defaults to `bypassPermissions`** (zero approval prompts in the isolated per-attendee container); full build stays at safe `default`. Override with `true`/`false` or an explicit mode (`acceptEdits`, `auto`, `plan`, `dontAsk`, `bypassPermissions`) |
